...
- `Unit` es un tipo de dato que hereda de `Any`.
...
- `Nothing` es un tipo de dato especial que representa la ausencia de un valor.

  *Ejemplo:*

  ```kotlin
  // Un uso común de Nothing es en una función que no tiene un punto de retorno normal.
  // Un ejemplo de tal función es throw Exception("Error") -> este tipo de funciones nunca retornan normalmente, pero interrumpen su ejecución lanzando un error.
...
- La función `TODO()` de la librería Kotlin tiene un tipo de retorno `Nothing`.

- La función `TODO()` está pensada para ser usada como un marcador de posición para código que no ha sido
implementado aún o como una forma de deshabilitar temporalmente código que no desea
ejecutar.

- `Nothing` es un subtipo de todos los otros tipos &rarr; por tanto se puede usar en lugares donde se espera cualquier otro tipo. Es decir, podemos usar TODO() en cualquier lugar, independientemente del tipo de retorno declarado por la función.

   ```kotlin
  fun calculateSomething(a: Int, b: Int): Int = TODO("Not yet implemented")
  ```
